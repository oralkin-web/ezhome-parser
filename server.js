...
async function parsePage(url) {
  ...
  try {
    ...
    const result = await page.evaluate(() => {
      ...
      // 1. Специфичные CSS классы для цены товара (самый надёжный)
      const priceSelectors = [
        ...
        '[class*="price__number"]', '[class*="price__amount"]',
      ];
      for (const sel of priceSelectors) {
        ...
        for (const el of els) {
          ...
          if (m) {
            ...
            if (p >= 1000 && p <= 10000000) { price = p; break; }
          }
        }
        if (price) break;
      }

      // 2. JSON-LD — только если нет из CSS
      ...
      // 3. Листовые элементы с ₽ — ищем все, берём максимальную разумную
      // (цена товара обычно самая большая на странице среди цен)
      if (!price) {
        const candidates = [];
        for (const el of document.querySelectorAll('*')) {
          if (el.children.length === 0 && /[\d\s]{4,12}[₽руб]/.test(el.innerText)) {
            ...
            if (m) {
              const p = parseInt(m[1].replace(/\s/g, ''));
              if (p >= 1000 && p <= 10000000) candidates.push(p);
            }
          }
        }
        if (candidates.length > 0) {
          // Берём первое вхождение среди топ-3 самых больших (цена товара обычно крупная)
      ...
      // 3. Строгие паттерны после метки
      if (!color) {
